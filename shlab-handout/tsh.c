...
{   
    ...
    pid_t id=(pid_t)atoi(buf);
   // printf("in do_gb_fg: id==%d\n",id);
    sigprocmask(SIG_BLOCK,&mask_all,&prev_all);
    if(is_job)
 ...
 */
//SIGCHILD is created when a state of child has changed
void sigchld_handler(int sig)
{   
    //printf("call sigchld_handler\n");
    sigset_t mask_all,prev_all;
    pid_t pid;
    int status;
    struct job_t* job=NULL;
